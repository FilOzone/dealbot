...
| <a id="spSyncedToIpni"></a>`spSyncedToIpni` | By polling SP, dealbot learned the SP itself confirms the ad was picked up by the indexer (`synced=true`). When the SP returns `syncedAt`, dealbot uses that SP timestamp; otherwise it uses the local poll time. Gates [`ipniVerificationStart`](#ipniVerificationStart) — dealbot never queries cid.contact before this. | Data Storage | [`ipni.strategy.ts`](../../apps/backend/src/deal-addons/strategies/ipni.strategy.ts) |
| <a id="ipniVerificationStart"></a>`ipniVerificationStart` | Dealbot begins polling cid.contact for a <IpfsRootCid,SP> provider record. Only occurs after the SP reports `synced=true`. This event anchors [`ipniVerifyMs`](#ipniVerifyMs) timer starts for Data Storage: prefer the SP's `syncedAt` when available and sane, otherwise the local time dealbot observed `synced=true` while polling the SP; if no sync timestamp exists, fall back to this polling-start time. Retrieval checks always anchor from this local polling-start time. | Data Storage, Retrieval | [`ipni.strategy.ts`](../../apps/backend/src/deal-addons/strategies/ipni.strategy.ts), [`ipni-verification.service.ts`](../../apps/backend/src/ipni/ipni-verification.service.ts) |
| <a id="ipniVerificationComplete"></a>`ipniVerificationComplete` | The cid.contact call completes (pass or timeout). Occurs once per check. | Data Storage, Retrieval | [`ipni.strategy.ts`](../../apps/backend/src/deal-addons/strategies/ipni.strategy.ts) |
...
sequenceDiagram
  ...
  participant IPNI as cid.contact
...
| <a id="sampledIpniVerificationStart"></a>`sampledIpniVerificationStart` | Dealbot begins polling cid.contact for `<rootCid + sampled CIDs, SP>` provider records.                                                                                                                                | [`piece-validation.service.ts`](../../apps/backend/src/sampled-retrieval/piece-validation.service.ts) |
...
| <a id="ipniVerifyMs"></a>`ipniVerifyMs` | Data Storage, Retrieval | [`ipniVerificationStart`](#ipniVerificationStart) | [`ipniVerificationComplete`](#ipniVerificationComplete) | One histogram observation per check. Extra labels: `value` (`success`, or `failure.*` — same vocabulary as [Discoverability Status](./data-storage.md#sub-status-meanings)) and `indexer` (always `cid.contact`). Timer-start rules: [ipniVerificationStart](#ipniVerificationStart). Note: the time the SP itself takes to report `synced=true` is tracked only on the deal entity (`ipniSyncedAt`/`ipniTimeToSyncMs`), not as a Prometheus histogram. | [`ipni.strategy.ts`](../../apps/backend/src/deal-addons/strategies/ipni.strategy.ts), [`retrieval.service.ts`](../../apps/backend/src/retrieval/retrieval.service.ts) |
...
| <a id="discoverabilityStatus"></a>`discoverabilityStatus` | Data Storage, Retrieval | [`ipniVerificationComplete`](#ipniVerificationComplete) | `success`, `failure.timedout`, `failure.other`, `skipped` from [Data Storage Sub-status meanings](./data-storage.md#sub-status-meanings). | Driven exclusively by the single cid.contact result. | 1 |  |
