...
| <a id="ipniVerificationComplete"></a>`ipniVerificationComplete` | An IPNI indexer call completes (pass or timeout). Occurs once per indexer: filecoinpin.contact always, cid.contact only when filecoinpin.contact verified. | Data Storage, Retrieval | [`ipni.strategy.ts`](../../apps/backend/src/deal-addons/strategies/ipni.strategy.ts) |
...
| <a id="discoverabilityStatus"></a>`discoverabilityStatus` | Data Storage, Retrieval | [`ipniVerificationComplete`](#ipniVerificationComplete) | `success`, `failure.timedout`, `failure.other` from [Data Storage Sub-status meanings](./data-storage.md#sub-status-meanings). `skipped` when IPNI verification is bypassed because `rootCID`/`blockCIDs` are absent from deal metadata or `rootCID` cannot be parsed as a valid CID. Driven exclusively by the filecoinpin.contact result; cid.contact outcome is tracked separately in [`cidContactStatus`](#cidContactStatus). |  |
| <a id="cidContactStatus"></a>`cidContactStatus` | Data Storage | [`ipniVerificationComplete`](#ipniVerificationComplete) for the cid.contact call | `verified` (root CID confirmed on cid.contact), `timeout` (duration ≥ timeout budget), `error` (other failure), `skipped` (filecoinpin.contact did not verify so cid.contact was never queried). | [`ipni.strategy.ts`](../../apps/backend/src/deal-addons/strategies/ipni.strategy.ts) |
