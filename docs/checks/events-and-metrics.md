...
| `dealbot_active_datasets` | Dataset inventory | Refreshed at most once every five minutes when Prometheus scrapes metrics. Labels: `network`, `providerId`, `providerName`, `providerStatus`. The collector explicitly paginates the FWSS view in 100-row pages instead of relying on SDK default fetch-all behavior. | Number of FWSS data sets owned by the configured network `WALLET_ADDRESS` with `pdpEndEpoch=0`. Terminated data sets are excluded immediately; the metric does not wait for rail finalization or PDP deletion. | FWSS `getClientDataSets` view via [`active-datasets.collector.ts`](../../apps/backend/src/metrics-prometheus/active-datasets.collector.ts) |
