...
| <a id="ipniVerifyMs"></a>`ipniVerifyMs` | Data Storage, Retrieval | [`ipniVerificationStart`](#ipniVerificationStart) | [`ipniVerificationComplete`](#ipniVerificationComplete) | For Data Storage, SP `advertisedAt` is preferred so this measures advertisement-to-filecoinpin.contact visibility. If the timestamp is missing or has obvious clock skew, dealbot falls back to its own IPNI polling duration. Retrieval checks always use dealbot's polling duration. | [`ipni.strategy.ts`](../../apps/backend/src/deal-addons/strategies/ipni.strategy.ts) |
