...
| <a id="ipniVerificationStart"></a>`ipniVerificationStart` | Dealbot begins polling filecoinpin.contact for <IpfsRootCid,SP> provider record. For Data Storage metric computation, `ipniVerifyMs` can use the earlier SP advertisement timestamp as its timer start when available and sane; the polling-start log remains local to dealbot. Retrieval always uses the local polling window. | Data Storage, Retrieval | [`ipni.strategy.ts`](../../apps/backend/src/deal-addons/strategies/ipni.strategy.ts), [`ipni-verification.service.ts`](../../apps/backend/src/ipni/ipni-verification.service.ts) |
...
| <a id="ipniVerifyMs"></a>`ipniVerifyMs` | Data Storage, Retrieval | [`ipniVerificationStart`](#ipniVerificationStart) | [`ipniVerificationComplete`](#ipniVerificationComplete) | Carries a `value` label: `verified` (root CID confirmed in IPNI), `timeout` (duration ≥ timeout budget), `error` (other failure). Filter `{value="verified"}` for a true latency SLI. For Data Storage, SP `advertisedAt` is preferred so this measures advertisement-to-filecoinpin.contact visibility. If the provider timestamp is missing or has obvious clock skew, dealbot anchors instead to the local time it observed `advertised=true` while polling the SP (still an advertisement-to-visible window). Only when no advertisement timestamp exists at all does it fall back to the raw IPNI polling duration (`ipniVerificationStart` to `ipniVerificationComplete`). Both fallback paths anchor from when dealbot *noticed* the advertisement rather than the SP's claimed time, so they lag by up to one SP poll interval and read shorter than provider-anchored samples; account for this when reading p90 if providers omit timestamps. Retrieval checks always use dealbot polling duration. | [`ipni.strategy.ts`](../../apps/backend/src/deal-addons/strategies/ipni.strategy.ts), [`retrieval.service.ts`](../../apps/backend/src/retrieval/retrieval.service.ts) |
