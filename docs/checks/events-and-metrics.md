...
|---|---|---|---|---|
| <a id="pdp_provider_estimated_overdue_periods"></a>`pdp_provider_estimated_overdue_periods` | Storage Proving | Emitted on every [Storage Proving Check](./data-retention.md) poll for every successfully processed provider. | Number of overdue proving periods (non-negative integer) | [`data-retention.service.ts`](../../apps/backend/src/data-retention/data-retention.service.ts) |
| `dealbot_active_datasets` | Dataset inventory | Refreshed in the background every hour on the API pod. Labels: `network`, `providerId`, `providerName`, `providerStatus`. | Number of FWSS data sets owned by the configured network `WALLET_ADDRESS` whose dealbot-owned subgraph record has `isActive=true`. `FWSS.ServiceTerminated` and `PDPVerifier.DataSetDeleted` both remove a data set from the count. | Dealbot-owned subgraph via [`active-datasets.collector.ts`](../../apps/backend/src/metrics-prometheus/active-datasets.collector.ts) |
| `dealbot_expected_active_datasets` | Dataset inventory | Emitted per active network during inventory refresh. Label: `network`. | Configured `MIN_NUM_DATASETS_FOR_CHECKS`. | Dealbot network configuration |
| `dealbot_active_datasets_last_success_timestamp_seconds` | Dataset inventory | Updated only after a successful inventory refresh. Label: `network`. | Unix timestamp in seconds; use it to alert when inventory data becomes stale. | [`active-datasets.collector.ts`](../../apps/backend/src/metrics-prometheus/active-datasets.collector.ts) |
| `dealbot_subgraph_indexed_block_number` | Subgraph health | Updated after a successful inventory refresh. Label: `network`. | Latest dealbot-owned subgraph block number observed across the paginated inventory query. Compare with chain head to detect indexing lag. | Dealbot-owned subgraph `_meta.block.number` |
