...
sequenceDiagram
  ...
  participant CNIC as cid.contact IPNI Instance
  ...
  opt filecoinpin.contact verified
    Dealbot->>CNIC: ipniVerificationStart (cid.contact)
    CNIC-->>Dealbot: ipniVerificationComplete (cid.contact)
  end
...
| <a id="ipniVerificationStart"></a>`ipniVerificationStart` | Dealbot begins polling an IPNI indexer for a <IpfsRootCid,SP> provider record. Occurs once per indexer call: first for filecoinpin.contact, then for cid.contact (only when filecoinpin.contact verified). For Data Storage metric computation, `ipniVerifyMs` can use the earlier SP advertisement timestamp as its timer start when available and sane; the polling-start log remains local to dealbot. Retrieval always uses the local polling window. | Data Storage, Retrieval | [`ipni.strategy.ts`](../../apps/backend/src/deal-addons/strategies/ipni.strategy.ts), [`ipni-verification.service.ts`](../../apps/backend/src/ipni/ipni-verification.service.ts) |
...
| <a id="ipniVerifyMs"></a>`ipniVerifyMs` | Data Storage, Retrieval | [`ipniVerificationStart`](#ipniVerificationStart) | [`ipniVerificationComplete`](#ipniVerificationComplete) | One observation is emitted per indexer call. Carries two extra labels: 1) `status`: either `success`, `skipped` or `failure.*` as defined in [Discoverability Status](./data-storage.md#sub-status-meanings); 2) `indexer`: either `filecoinpin.contact` or `cid.contact`. Filter `{value="success", indexer="filecoinpin.contact"}` for a true latency SLI around SP and IPNI interaction. | [`ipni.strategy.ts`](../../apps/backend/src/deal-addons/strategies/ipni.strategy.ts), [`retrieval.service.ts`](../../apps/backend/src/retrieval/retrieval.service.ts) |
...
| <a id="cidContactVerification"></a>`cidContactVerification` | Data Storage, Retrieval | [`ipniVerificationComplete`](#ipniVerificationComplete) for the cid.contact call |  `success`, `failure.timedout`, `failure.other` from [Data Storage Sub-status meanings](./data-storage.md#sub-status-meanings). | Only emitted if discoverabilityStatus is `success` | 1 | [`ipni.strategy.ts`](../../apps/backend/src/deal-addons/strategies/ipni.strategy.ts) |
...
| <a id="dataSetChallengeStatus"></a>`dataSetChallengeStatus` | Data Retention | Emitted on each [Data Retention Check](./data-retention.md) poll when a provider's confirmed proving-period totals advance (strictly positive deltas). | `success` (challenges in successfully-proven periods), `failure` (challenges in faulted periods) |  | Unit: **challenges** (period delta × `CHALLENGES_PER_PROVING_PERIOD = 5`). | [`data-retention.service.ts`](../../apps/backend/src/data-retention/data-retention.service.ts) |
