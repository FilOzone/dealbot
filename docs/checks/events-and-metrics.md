...
| <a id="ipniVerifySkippedTotal"></a>`ipniVerifySkippedTotal` | Data Storage | When IPNI verify is skipped because there are no valid CIDs to check — either `rootCID`/`blockCIDs` are absent or `rootCID` cannot be parsed. | n/a (no `value` label; each increment counts one skipped verify attempt) | [`ipni.strategy.ts`](../../apps/backend/src/deal-addons/strategies/ipni.strategy.ts) |
