...
| <a id="dataSetLifecycleCheckMs"></a>`dataSetLifecycleCheckMs` | Data-Set Lifecycle Check (empty variant) | Empty data set creation start | `terminateServiceSync` tx receipt received | End-to-end duration of the empty variant: `createDataSet` → `waitForCreateDataSet` → `terminateServiceSync`. Emitted on `success` and `failure.timedout` only. See [data-set-lifecycle-check.md](./data-set-lifecycle-check.md). | [`data-set-lifecycle.service.ts`](../../apps/backend/src/data-set-lifecycle/data-set-lifecycle.service.ts) |
| <a id="dataSetWithPiecesLifecycleCheckMs"></a>`dataSetWithPiecesLifecycleCheckMs` | Data-Set Lifecycle Check (with-pieces variant) | Canary piece upload start | `terminateServiceSync` tx receipt received | End-to-end duration of the with-pieces variant: `uploadPieceStreaming` → `findPiece` → `createDataSetAndAddPieces` → `waitForCreateDataSetAddPieces` → `terminateServiceSync`. Emitted on `success` and `failure.timedout` only. See [data-set-lifecycle-check.md](./data-set-lifecycle-check.md). | [`data-set-lifecycle.service.ts`](../../apps/backend/src/data-set-lifecycle/data-set-lifecycle.service.ts) |
...
| <a id="dataSetLifecycleCheckStatus"></a>`dataSetLifecycleCheckStatus` | Data-Set Lifecycle Check (empty variant) | When an empty-variant `data_set_lifecycle_check` invocation finishes | `success`, `failure.timedout`, `failure.other` | `success` confirms the full `createDataSet` → `terminateService` lifecycle completed. Persistent `failure.*` indicates a `createDataSet` or `terminateService` regression on the empty path. See [data-set-lifecycle-check.md](./data-set-lifecycle-check.md). | 1 | [`data-set-lifecycle.service.ts`](../../apps/backend/src/data-set-lifecycle/data-set-lifecycle.service.ts) |
| <a id="dataSetWithPiecesLifecycleCheckStatus"></a>`dataSetWithPiecesLifecycleCheckStatus` | Data-Set Lifecycle Check (with-pieces variant) | When a with-pieces-variant `data_set_lifecycle_check` invocation finishes | `success`, `failure.timedout`, `failure.other` | `success` confirms the full `uploadPieceStreaming` → `findPiece` → `createDataSetAndAddPieces` → `terminateService` lifecycle completed. Persistent `failure.*` indicates a regression on the with-pieces path. See [data-set-lifecycle-check.md](./data-set-lifecycle-check.md). | 1 | [`data-set-lifecycle.service.ts`](../../apps/backend/src/data-set-lifecycle/data-set-lifecycle.service.ts) |
