...
| Job type | `deal`, `retrieval`, `data_set_creation`, `data_set_termination`, `piece_cleanup`, `pull_check`, `providers_refresh`, `data_retention_poll` | `deal` corresponds to "data storage check" externally; we keep `deal` in code/DB for compatibility. |
...
| `data_set_termination` | `sp.work` | [`JobsService.handleDataSetTerminationJob`](../apps/backend/src/jobs/jobs.service.ts) | `{ jobType: 'data_set_termination', spAddress, intervalSeconds }` | [data-set-termination](./data-set-termination.md) |
