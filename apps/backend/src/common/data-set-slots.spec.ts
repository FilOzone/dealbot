...
import { getBaseDataSetMetadata, metadataMatchesExactly, storedSlotMetadata } from "./data-set-slots.js";
...
describe("storedSlotMetadata", () => {
   ...
   *
   * This is the contract pruning depends on. `metadataMatchesExactly` requires an identical key
  ...
  it("matches a real on-chain metadata object exactly", () => {
    // Data set 729 on mainnet, verbatim.
    const onChain = { dealbotDS: "1", source: "dealbot", withIPFSIndexing: "" };
    expect(metadataMatchesExactly(onChain, storedSlotMetadata(getBaseDataSetMetadata(config), 1))).toBe(true);
    expect(metadataMatchesExactly(onChain, storedSlotMetadata(getBaseDataSetMetadata(config), 2))).toBe(false);
  });
