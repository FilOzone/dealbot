...
describe("RetrievalService DB/provider drift", () => {
  ...
  it("selectRandomDealsForRetrieval excludes cleaned-up deals", async () => {
    const { qb, calls } = createMockQueryBuilder();
    const svc = (await createServiceWithQb(qb)) as unknown as {
      selectRandomDealsForRetrieval: (count: number) => Promise<Deal[]>;
    };

    await svc.selectRandomDealsForRetrieval(5);
