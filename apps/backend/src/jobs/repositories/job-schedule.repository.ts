...
export class JobScheduleRepository {
    ...
    try {
      if (activeAddresses.length === 0) {
        ...
        const [rows] = (await this.dataSource.query(
          ...
          WHERE job_type IN ('deal', 'retrieval', 'retrieval_anon', 'data_set_creation', 'data_set_lifecycle_check', 'piece_cleanup', 'pull_check')
            AND sp_address <> ''
   ...
   */
  async deleteSchedulesByJobType(jobType: JobType): Promise<number> {
