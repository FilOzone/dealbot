...
describe("SampledPieceSelectorService", () => {
  let subgraphService: SubgraphService;
  let samplePiece: ReturnType<typeof vi.fn>;

  beforeEach(() => {
    samplePiece = vi.fn();
    subgraphService = { samplePiece } as unknown as SubgraphService;
  });
  ...
  it("returns null without sampling when the signal is already aborted", async () => {
    samplePiece.mockResolvedValue(makePiece());
    const service = new SampledPieceSelectorService(subgraphService, makeConfigService());
  ...
  it("passes the dealbot payer address to samplePiece for exclusion", async () => {
